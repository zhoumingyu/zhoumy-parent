...
import org.mybatis.generator.config.JavaVoModelGeneratorConfiguration;
...
public abstract class IntrospectedTable {
	...
	protected enum InternalAttribute {
		...
		/** The attr base query record type */
		ATTR_BASE_QUERY_RECORD_TYPE,
		...
		/** The ATT Service Impl */
		ATTR_SERVICE_IMPLEMENTATION_TYPE,
		/** The ATT Service */
		ATTR_SERVICE_INTERFACE_TYPE, ATTR_Controller_IMPLEMENTATION_TYPE
	}
	...
	public String getBaseQueryRecordType() {
		return internalAttributes.get(InternalAttribute.ATTR_BASE_RECORD_TYPE);
	}
	...
	public String getControllerImplementationType() {
		return internalAttributes.get(InternalAttribute.ATTR_Controller_IMPLEMENTATION_TYPE);
	}
	 ...
	 * 
	 * @author zhoumingyu
	 * @since 2017年6月5日 上午10:28:23
	 * @company 万顺叫车深圳云信息技术有限公司
	 * @version 1.0
	 */
	protected void calculateServiceAttributes() {
		...
		sb.append(calculateJavaServiceImplementationPackage(fullyQualifiedTable.getDomainObjectName()));
		sb.append('.');
		...
		sb.setLength(0);
		sb.append(calculateJavaServiceInterfacePackage(fullyQualifiedTable.getDomainObjectName()));
		sb.append('.');
		sb.append(fullyQualifiedTable.getDomainObjectName());
	...
	protected void calculateControllerAttributes() {
		...
		StringBuilder sb = new StringBuilder();
		sb.append(calculateJavaControllerImplementationPackage());
		sb.append('.');
		sb.append(fullyQualifiedTable.getDomainObjectName());
		sb.append("Controller"); //$NON-NLS-1$
		setControllerImplementationType(sb.toString());
	}
	 ...
	 * @return the string
	 */
	protected String calculateJavaClientImplementationPackage(String className) {
		...
		if (stringHasValue(config.getImplementationPackage())) {
			...
			sb.append(String.format(config.getTargetPackage(), className.toLowerCase()));
		}
	...
	protected String calculateJavaControllerImplementationPackage() {
	...
	protected String calculateJavaServiceImplementationPackage(String classname) {
		...
		StringBuilder sb = new StringBuilder();
		if (stringHasValue(config.getImplementationPackage())) {
			sb.append(String.format(config.getImplementationPackage(), classname.toLowerCase()));
		} else {
			sb.append(String.format(config.getTargetPackage(), classname.toLowerCase()));
		}
	 ...
	 * @return the string
	 */
	protected String calculateJavaClientInterfacePackage(String classname) {
		JavaClientGeneratorConfiguration config = context.getJavaClientGeneratorConfiguration();
		if (config == null) {
			return null;
		}
	...
	protected String calculateJavaServiceInterfacePackage(String classname) {
		JavaServiceGeneratorConfiguration config = context.getJavaServiceGeneratorConfiguration();
		if (config == null) {
			return null;
		}
	...
	protected void calculateJavaClientAttributes() {
		...
		StringBuilder sb = new StringBuilder();
		sb.append(calculateJavaClientImplementationPackage(fullyQualifiedTable.getDomainObjectName()));
		sb.append('.');
		sb.append(fullyQualifiedTable.getDomainObjectName());
		...
		sb.setLength(0);
		sb.append(calculateJavaClientInterfacePackage(fullyQualifiedTable.getDomainObjectName()));
		sb.append('.');
		sb.append(fullyQualifiedTable.getDomainObjectName());
	...
	protected void calculateVoModelAttributes() {
		String pakkage = calculateJavaVoModelPackage();
		...
		sb.append(fullyQualifiedTable.getDomainObjectName());
		sb.append("Query");
		setBaseQueryRecordType(sb.toString());
	}
	...
	protected String calculateSqlMapPackage() {
		...
		return String.format(sb.toString(), tableConfiguration.getDomainObjectName().toLowerCase());
	}
	...
	public abstract List<GeneratedJspFile> getGeneratedJspFiles();
	...
	public void setControllerImplementationType(String ControllerImplementationType) {
		internalAttributes.put(InternalAttribute.ATTR_Controller_IMPLEMENTATION_TYPE, ControllerImplementationType);
	}
	...
	public void setBaseQueryRecordType(String baseQueryRecordType) {
		internalAttributes.put(InternalAttribute.ATTR_BASE_QUERY_RECORD_TYPE, baseQueryRecordType);
	}
