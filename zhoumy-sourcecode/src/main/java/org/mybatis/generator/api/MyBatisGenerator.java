...
public class MyBatisGenerator {
	...
	/** The generated jsp files. */
	private List<GeneratedJspFile> generatedJspFiles;
	...
	public MyBatisGenerator(Configuration configuration, ShellCallback shellCallback, List<String> warnings)
		...
		generatedJspFiles = new ArrayList<>();
	...
	public void generate(ProgressCallback callback, Set<String> contextIds, Set<String> fullyQualifiedTableNames,
		...
		for (Context context : contextsToRun) {
			context.generateFiles(callback, generatedJspFiles, generatedJavaFiles, generatedXmlFiles, warnings);
		}
		...
		if (writeFiles) {
			...
			for (GeneratedJspFile gjf : generatedJspFiles) {
				projects.add(gjf.getTargetProject());
				writeGeneratedJspFile(gjf, callback);
			}
	...
	private void writeGeneratedJspFile(GeneratedJspFile gjf, ProgressCallback callback)
			throws InterruptedException, IOException {
		File targetFile;
		String source;
		try {
			File directory = shellCallback.getDirectory(gjf.getTargetProject(), gjf.getTargetPackage());
			File tarDir = new File(directory, gjf.getDirName());
			if (!tarDir.exists()) {
				tarDir.mkdirs();
			}
			targetFile = new File(directory, gjf.getFileName());
