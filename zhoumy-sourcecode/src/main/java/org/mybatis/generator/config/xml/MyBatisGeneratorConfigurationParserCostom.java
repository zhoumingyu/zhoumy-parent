...
import org.mybatis.generator.config.JavaJspGeneratorConfiguration;
 ...
 * @author Jeff Butler
 */
public class MyBatisGeneratorConfigurationParserCostom {
	private Properties extraProperties;
	private Properties configurationProperties;

	public MyBatisGeneratorConfigurationParserCostom(Properties extraProperties) {
		super();
	...
	private void parseContext(Configuration configuration, Node node) {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			} else if ("javaVoModelGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
				parseJavaVoModelGenerator(context, childNode);
			...
			} else if ("javaJspGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
				parseJavaJspGenerator(context, childNode);
			}
	...
	protected void parseSqlMapGenerator(Context context, Node node) {
		...
		if (targetProject == null || targetProject.equals("")) {
			targetProject = System.getProperty("user.dir") + "/target";
		}
	...
	protected void parseTable(Context context, Node node) {
		...
		if (domainObjectName == null || domainObjectName.equals("")) {
			domainObjectName = getCamelCaseString(tableName, true);
		}
	...
	protected void parseJavaVoModelGenerator(Context context, Node node) {
		JavaVoModelGeneratorConfiguration javaVoModelGeneratorConfiguration = new JavaVoModelGeneratorConfiguration();

		context.setJavaVoModelGeneratorConfiguration(javaVoModelGeneratorConfiguration);
		...
		javaVoModelGeneratorConfiguration.setTargetPackage(targetPackage);
		javaVoModelGeneratorConfiguration.setTargetProject(targetProject);
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
				parseProperty(javaVoModelGeneratorConfiguration, childNode);
			}
		}
	}
	...
	private void parseJavaJspGenerator(Context context, Node node) {
		JavaJspGeneratorConfiguration javaControllerGeneratorConfiguration = new JavaJspGeneratorConfiguration();

		context.setJavaJspGeneratorConfiguration(javaControllerGeneratorConfiguration);
